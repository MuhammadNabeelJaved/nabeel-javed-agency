...
/** GET /api/v1/announcements/bars — public, returns active bars with their active announcements */
export const getActiveBars = asyncHandler(async (req, res) => {
    await ensureDefaultBar();
    const bars = await AnnouncementBar.find({ isActive: true }).sort({ order: 1, createdAt: 1 }).lean();
    ...
    // Filter out bars with no active announcements
...
export const createBar = asyncHandler(async (req, res) => {
    const { name, bgColor, textColor, scrollEnabled, tickerDuration, textAlign, separatorVisible, separatorColor, itemSpacing, order } = req.body;
...
export const updateBar = asyncHandler(async (req, res) => {
    const allowed = [
        ...
        "separatorVisible", "separatorColor", "itemSpacing", "order", "isActive",
    ];
    const updates = {};
