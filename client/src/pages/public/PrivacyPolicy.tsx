/**
 * Privacy Policy Page
 * Content fully driven by CMS (ContentContext.privacyPolicy).
 */
import React from 'react';
import { motion } from 'framer-motion';
import { Shield, FileText, Eye, Server, Lock, Star } from 'lucide-react';
import { useContent } from '../../contexts/ContentContext';

const SECTION_ICONS = [FileText, Eye, Server, Lock, Shield, Star];
const SECTION_COLORS = [
  ...
  { icon: 'text-cyan-500',   bg: 'bg-cyan-500/10'   },
];

export default function PrivacyPolicy() {
  const { privacyPolicy } = useContent();
  const { lastUpdated, subtitle, contactEmail, sections } = privacyPolicy;
